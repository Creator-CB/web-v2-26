...
const HeroActiveInvestors = () => {
  ...
  return (
    <div className={`${isMobile ? 'relative mt-4 mx-auto' : 'absolute bottom-6 sm:bottom-10 left-1/2 transform -translate-x-1/2'} 
      bg-black/50 backdrop-blur-md shadow-lg rounded-full px-2 sm:px-6 py-2 sm:py-3 
      flex items-center gap-1 sm:gap-3 border border-amber-500/30 
      w-auto max-w-[90vw] sm:max-w-max z-20`}>
      <div className="relative flex-shrink-0">
        <div className="w-2 h-2 sm:w-3 sm:h-3 bg-green-500 rounded-full animate-pulse"></div>
        <div className="absolute inset-0 bg-green-500 rounded-full animate-ping opacity-75"></div>
      </div>
      <span className="font-medium text-gray-300 text-[10px] sm:text-base whitespace-nowrap overflow-visible">
        {isMobile ? (
          <>
